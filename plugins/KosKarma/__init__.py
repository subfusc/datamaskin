...
class Plugin(object):
    ...
    def cmd(self, command, args, channel, **kwargs):
        ...
        if command == "hikarma" or command == "high":
            ...
            best = ', '.join(["{}: {:.2f}".format(e, k[0]) for e,k in best])
        ...
        if command == "høy" or command == "snill":
            if args:
                ...
                best = self.backend(channel).getNBestList()
            best = ', '.join(["{}: {:.1f}".format(e, k[0]) for e,k in best])
        ...
        if command == "lav" or command == "slem":
            ...
            worst = ', '.join(["{}: {:.1f}".format(e, k[0]) for e,k in worst])
        ...
        if command == "lokarma" or command == "low":
            if args:
                ...
                worst = self.backend(channel).getNWorstList()
            worst = ', '.join(["{}: {:.2f}".format(e, k[0]) for e,k in worst])
            return [(1, channel, "bad karma things in {} - {}".format(channel, worst))]

        if command == "rmkarma":
            args_split = args.split()
            if kwargs['admin'] and len(args_split) >= 2:
                self.backend(args_split[0]).delEntity(' '.join(args_split[1:]))
        ...
        for karmatoken in self.reg.findall(msg):
            ...
            if match[0].startswith("++") or match[0].endswith("++"):
                if match[0].strip("++") != kwargs['from_nick']:
                    self.backend(channel).positiveKarma(match[0].strip("++"))

            if match[0].startswith("--") or match[0].endswith("--"):
                self.backend(channel).negativeKarma(match[0].strip("--"))
                ...
                if len(match) > 2:
                    ...
                    if number <= 3 and number > 0 and match[1] != kwargs['from_nick']:
                        for i in range(1, number):
                            self.backend(channel).positiveKarma(match[1])
                    else:
            ...
            if match[0].startswith("(decf"):
                if len(match) > 2:
                    ...
                    if number <= 3 and number > 0 and match[1] != kwargs['from_nick']:
                        for i in range(1, number):
                            self.backend(channel).negativeKarma(match[1])
                    else:
                        return [(1, kwargs['from_nick'],
...
if __name__ == '__main__':
    k = Plugin()
    k.listen('(decf microsoft)', '#foobar', from_nick='gitlab')
    print(k.cmd('karmaprec', 'microsoft', '#foobar', from_nick='gitlab'))
    print(k.cmd('rmkarma', '#foobar microsoft', '#foobar', from_nick='gitlab', admin=True))
    print(k.cmd('karmaprec', 'microsoft', '#foobar', from_nick='gitlab'))
