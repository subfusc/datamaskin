...
(defn cmd-preprocess [self message :protocol-context protcol-context]
  (setv split-cmd (.split (cut message 1 -1) " "))
  (.cmd self
        (get split-cmd 0)
        (.join " " (cut split-cmd 1))
        :admin protocol-context.admin
        :context (Context message self.nick protocol-context)))
